...
;; if the type is known, return a symbol
;; if there is a * return * and go deeper to find next type
;; if one of the types is unknown return nil
(defun string->type (ls)
  (cond 
    ((equal (car ls) '*) (let ((subtype (string->type (car (cdr ls)))))
			   (when subtype
			     (list '* subtype))))
    ((equal '("char") ls) 'char)
...
;; create calls for define-alien-routine but remove functions with
...
(defun hash-def (ls)
  "Write out code for a function that defines a hash table for error
  lookup."
  `(progn
     (defparameter *drv-hash* (make-hash-table))
     (dolist (e ',ls)
...
(with-open-file (s "autogenerated-ffi.lisp" 
		   :direction :output
		   :if-exists :supersede
